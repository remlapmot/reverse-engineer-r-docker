...
docker run --platform linux/amd64 -v "/${PWD}:/out" legacy-04-rstudio \
    R -e "write.csv(installed.packages()[, c('Package', 'Version')], \
                  row.names = FALSE, \
...
# Note the syntax for $PWD only works on a Unix shell or Git Bash on Windows (or within WSL)
docker run --platform linux/amd64 -v "/${PWD}:/out" legacy-04-rstudio \
    R -e "ip <- installed.packages()[, c('Package', 'Version')]; \
...
# docker run --platform linux/amd64 legacy-04-rstudio \
#    -e "$tests"
