...
docker run --platform linux/amd64 -v "/${PWD}:/out" legacy-01 \
    -e "write.csv(installed.packages()[, c('Package', 'Version')], \
                  row.names = FALSE, \
                  file = '/out/packages-legacy-01.csv')"
...
docker run --platform linux/amd64 -v "/${PWD}:/out" legacy-01 \
    -e "ip <- installed.packages()[, c('Package', 'Version')]; \
        write.csv(ip[stringr::str_sort(ip[, 'Package']),], \
                  row.names = FALSE, \
                  file = '/out/packages-alphabetical-legacy-01.csv')"

# Load all packages
tests=$(grep -v '^#' packages.txt | awk '{ print $1 }' | xargs -L1 -I {} echo "library(\"{}\", warn.conflicts = FALSE)")
docker run --platform linux/amd64 legacy-01 \
    -e "$tests"
