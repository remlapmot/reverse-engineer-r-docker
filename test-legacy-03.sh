...
docker run --platform linux/amd64 -v "/${PWD}:/out" legacy-03 \
    -e "write.csv(installed.packages()[, c('Package', 'Version')], \
                  row.names = FALSE, \
                  file = '/out/packages-legacy-03.csv')"
...
docker run --platform linux/amd64 -v "/${PWD}:/out" legacy-03 \
    -e "ip <- installed.packages()[, c('Package', 'Version')]; \
        write.csv(ip[stringr::str_sort(ip[, 'Package']),], \
                  row.names = FALSE, \
                  file = '/out/packages-alphabetical-legacy-03.csv')"

# Load all packages
tests=$(grep -v '^#' packages.txt | awk '{ print $1 }' | xargs -L1 -I {} echo "library(\"{}\", warn.conflicts = FALSE)")
docker run --platform linux/amd64 legacy-03 \
    -e "$tests"
...
# interactive session
# docker run --platform linux/amd64 --entrypoint bash -it legacy-03

# test code
# docker run --platform linux/amd64 legacy-03 -e "remove.packages(c('MASS', 'mgcv', 'nlme', 'spatial', 'survival'), lib = '/usr/local/lib/R/library')"
